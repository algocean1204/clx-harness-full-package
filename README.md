...
사용자가 맡긴 현재 프로젝트 작업에서는 프로젝트 루트 아래의 파일·폴더 생성, 읽기, 수정, 이동,
이름 변경, 삭제를 추가 대화 승인 없이 바로 실행합니다. 프로젝트 루트 자체와 `.git`, 비밀·인증
파일, 다른 프로젝트나 외부 경로, 원격 서비스, 배포·공개, Git 이력 파괴 작업은 이 권한에 포함하지
않으며 기존 보호 절차를 그대로 적용합니다.

## 필요한 시점에만 쓰는 명세

비단순 새 작업은 먼저 `작업명세서 승인`을 받습니다. 요구사항 `W1`, 결과물 `O1`, 완료 기준
`A1`, 제외 항목 `X1`처럼 문자와 숫자를 붙여 사용자가 의도와 누락을 정확히 대조할 수 있게 합니다.
단순 답변과 이미 승인된 범위의 정확한 후속 작업은 생략합니다.

파일·코드·설정 변경, 배포·공개, 보호 명령이 있는 작업만 실행 직전에 `개발명세서 승인`을
한 번 더 받습니다. `D1 [W1]`은 어떤 요구를 구현하는지, `V1 [A1]`은 무엇으로 완료를 증명하는지
연결합니다. 읽기 전용 분석에는 개발명세서를 만들지 않고 `N/A`로 처리합니다. 두 명세서는 채팅
응답이며 별도 파일이나 버전 표기를 만들지 않습니다.

실행 중에는 명세서를 반복하지 않고 측정 결과만 짧게 알립니다. 목표·범위·권한·위험·완료 기준이
달라지면 작업명세서로 돌아가고, 구현 방법만 달라지면 개발명세서로 돌아갑니다.

### 채팅 완료 보고

비단순 작업은 마지막에 한 번만 `작업명세 N/N`, `개발명세 N/N` 또는 `개발명세 N/A`, DoD,
...
force-push나 hard reset처럼 가드가 막는 작업은 **소유자만** 열 수 있습니다. 에이전트는 적용 가능한
명세서에서 정확한 명령·동작·복구 방법을 먼저 설명하고, 나머지 안전한 일을 끝낸 뒤 짧은 승인 코드를
마지막에 제공합니다. 승인 챌린지만 단독으로 보내지 않습니다.

```
승인:7F3C22
```

이 **한 줄만** 그대로 보내면 `UserPromptSubmit` 훅이 해당 ID에 미리 결합된 명령의 SHA-256을
...
- **코드는 명령과 결합됩니다.** 명령이 달라지면 기존 코드는 쓸 수 없고 새 코드를 발급해야 합니다.
  사용자는 코드 발급 전에 작업명세서·개발명세서에서 전체 명령과 영향을 확인합니다.
...
- **AI 공통 루트**: 사람이 수정하는 공통 정책 원본은 `~/.agents/AGENTS.md` 하나입니다. Claude는
  직접 가져오고, Codex는 일치 검사된 공통 코어를 사용하며, Grok 어댑터는 실행 전에 그 파일을
  명시적으로 읽습니다. 플랫폼이 훅·샌드박스·서브에이전트를 지원하지 않으면 같은 기능을 가장하지
  않고 미지원 사실을 알립니다. 격리 Grok에는 세션 상태 대신 공통 루트의 읽기 전용 사본만 전달합니다.
