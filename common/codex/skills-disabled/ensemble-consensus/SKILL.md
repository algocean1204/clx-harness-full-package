...
description: Run bounded readonly adversarial debate, orchestrator consensus, and authorized same-turn implementation with verification. Use for explicit 3-model consensus, 앙상블, 다관점, material reversible high-stakes ambiguity after spec lock, or a work/intent INTERNAL_GATE decision.
---

# Ensemble Consensus (Codex)

Policy: `~/.codex/guides/work/ensemble-consensus.md`. All lenses run as **readonly
subagents on the pinned main model** — Grok is wired but NOT a routing default (see `work/models`); never route debate
or implementation to it. Implementation: orchestrator direct (light) or one write-capable
subagent (heavy).

## Roles — always 3, parallel readonly subagents
...
Orchestrator (pinned main model) merges, owns `<consensus_decision>` and the reply.
...
[ ] 3 EXECUTE   — implement SAME TURN (direct or one subagent, spec carried verbatim)
...
- Routing debate/implementation to Grok (manual picker use only)
