...
Load when: user asks 3-model consensus / 앙상블 / 다관점 / 3모델 합의; material reversible
...
- Use actual distinct transports when explicitly requested and available. If a requested transport is
  ...
  on the pinned main model and call it 3-lens, never 3-model. **Grok is never routed** to debate or
  implementation (see `work/models`). Orchestrator owns `<consensus_decision>` and the reply.
