> **OVERRIDE NOTICE (clx fork):** Local policy overrides in ../SKILL.md WIN over anything below — tier/concurrency caps (2 tiers max, 10 concurrent max), agent teams NOT default, never auto-edit CLAUDE.md/AGENTS.md, lean bias, outputs stay inside the target repo only, no external CLIs, no model pins.
