...
- `/grok` → Grok Build CLI; model/effort from `~/.agents/models.toml` role `grok_exec`. Repo execution: `clx-grok-delegate <repo> -p "<prompt>" --always-approve --output-format plain`. One-shot second opinion: `clx-grok-call` CLI, invoked directly via Bash (the wrapper plugin is retired as a duplicate of this path).
...
- A bare executor preference does not bypass the automatic gate; an explicit assignment does. Keep transient role bindings in this session's UUID-owned intent core.
